...
#     parser.add_argument("--maxiter", default=1, type=int, help='(int): maximum number of iterations')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--layers", default=3, type=int, help='(int): number of ansatz layers')
    parser.add_argument("--example", default=0, type=int, help='(int): 0 to run Ising model example or 1 to run the Handwritten digits example')
    parser.add_argument("--maxiter", default=10000, type=int, help='(int): maximum number of iterations')
    args = parser.parse_args()
    
    costs_efqae = main(0, **vars(args))
    costs_qae = main(1, **vars(args))
    
    fig, ax = plt.subplots(figsize=(12,6))
    ax.plot(np.arange(0, len(costs_efqae), 1), costs_efqae, color='b', label='EF_QAE')
    ax.plot(np.arange(0, len(costs_qae), 1), costs_qae, color='r', label='QAE')
    plt.yscale('log')
    ax.set_xlabel("Steps")
    ax.set_ylabel("Loss")
    ax.set_title("Autoencoder Loss Progression")
    plt.legend()
    #plt.savefig("QADsim/mnist_loss_plot.pdf", format="pdf")
    plt.savefig("QADsim/ising_loss_plot.pdf", format="pdf")
    plt.show()
